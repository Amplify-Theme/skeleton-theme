...
jQuery(document).ready(function ($) {
    ...
    function stickyHeader() {
        ...
        var $siteMenuH = $('.site-menu').outerHeight();

        if ($navbar.length) {
            $(window).on('scroll', function () {
                if ($(this).scrollTop() > $topbarH + $siteMenuH + 100) {
    ...
    function sidebarMenuOpen(e) {
        ...
        e.preventDefault();
        $('body').addClass('cs-overlay-none');
    }

    function sidebarMenuClose() {
        var $sidebar = $('.sidebar-offcanvas');
        $sidebar.removeClass('offcanvas-open');
        setTimeout(function () {
            $sidebar.css('overflow', 'visible');
            $('.sidebar-offcanvas').removeClass('open');
        }, 450);
        $('body').removeClass('cs-overlay-none');
    }

    $('.sidebar-toggle').on('click', sidebarMenuOpen);
    $('.sidebar-close').on('click', sidebarMenuClose);
    ...
    $.validator.setDefaults({
        ...
        errorPlacement: function (error, element) {
            element.closest('.form-group').find(".invalid-feedback:first").html(error.text());
        },
